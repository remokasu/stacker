...
class OperatorManager:
    ...
    def is_operator(self, operator: str) -> bool:
        if operator in self.oprerators["priority"]:
            return True
        if operator in self.oprerators["regular"]:
            return True
        if operator in self.oprerators["hof"]:
            return True
        if operator in self.oprerators["aggregate"]:
            return True
        if operator in self.oprerators["transform"]:
            return True
        if operator in self.oprerators["stack"]:
            return True
    ...
    def get_all_keys(self) -> list[str]:
        ...
        for kind in self.oprerators.keys():
            key += list(self.oprerators[kind].keys())
        return key
